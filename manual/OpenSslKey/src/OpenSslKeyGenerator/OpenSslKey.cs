...
namespace OpenSslKeyGenerator
{
    ...
    public class opensslkey
    {
        ...
        public static void WriteKeyXml(string xmlString, KeyType keyType)
        {
            string fileName = Path.Combine(".", string.Format("{0}.xml", keyType));
            byte[] buffer = Encoding.Default.GetBytes(xmlString);

            PutFileBytes(fileName, buffer, buffer.Length);
            Console.WriteLine("Wrote key out to {0}", Path.GetFullPath(fileName));
        }
        ...
        public static void DecodePEMKey(String pemstr)
        {
            ...
            if (pemstr.StartsWith(pempubheader) && pemstr.EndsWith(pempubfooter))
            {
                ...
                if (pempublickey != null)
                {
                    ...
                    WriteKeyXml(xmlpublickey, KeyType.PublicKey);
                }
            }
            ...
            else if (pemstr.StartsWith(pemprivheader) && pemstr.EndsWith(pemprivfooter))
            {
                ...
                if (pemprivatekey != null)
                {
                    ...
                    WriteKeyXml(xmlprivatekey, KeyType.PrivateKey);
                    ProcessRSA(rsa);
                }
            }
        ...
        public static void RSAtoPKCS12(RSACryptoServiceProvider rsa)
        {
            ...
            var keyspec = (uint) keyInfo.KeyNumber;
        ...
        private static byte[] GetPkcs12(RSA rsa, String keycontainer, String cspprovider, uint KEYSPEC, uint cspflags)
        {
            ...
            try
            {
                var cert = new X509Certificate(hCertCntxt); //create certificate object from cert context.
                //todo: Add reference to System.Security
        ...
        private static IntPtr CreateUnsignedCertCntxt(String keycontainer, String provider, uint KEYSPEC, uint cspflags, String DN)
        {
            ...
            var subjectblob = new CERT_NAME_BLOB();
            ...
            hCertCntxt = Win32.CertCreateSelfSignCertificate(IntPtr.Zero, ref subjectblob, CERT_CREATE_SELFSIGN_NO_SIGN, ref pInfo, IntPtr.Zero, IntPtr.Zero,
        ...
        {
            var myEx = new Win32Exception(errorcode);
