...
#try { #error handling is only necessary if you need to do anything in addition to/instead of the main helpers
  ...
  Install-ChocolateyPackage '__NAME__' 'EXE_OR_MSI' 'SILENT_ARGS' '{{DownloadUrl}}' '64BIT_URL_DELETE_IF_NO_64BIT'  -validExitCodes @(0)
  ...
  Install-ChocolateyZipPackage '__NAME__' '{{DownloadUrl}}' "$(Split-Path -parent $MyInvocation.MyCommand.Definition)" '{{DownloadUrlx64}}' 
  
  # other helpers - using any of these means you want to uncomment the error handling up top and at bottom.
  # downloader that the main helpers use to download items
  #Get-ChocolateyWebFile '__NAME__' 'DOWNLOAD_TO_FILE_FULL_PATH' '{{DownloadUrl}}' '{{DownloadUrlx64}}'
